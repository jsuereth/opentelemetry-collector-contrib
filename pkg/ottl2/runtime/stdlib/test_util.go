...
func lookupTestPath(v runtime.Val, path []testPath) runtime.Val {
	result := v
	for _, p := range path {
		switch {
		// TODO - staticly enforce these casts *or* somehow better error.
		case p.name != nil:
			result = result.(runtime.Structure).GetField(*p.name)
		case p.key != nil:
			result = result.(runtime.KeyIndexable).GetKey(*p.key)
		case p.index != nil:
			result = result.(runtime.Indexable).GetIndex(*p.index)
		}
	}
	return result
}
